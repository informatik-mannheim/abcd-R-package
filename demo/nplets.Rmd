...
skews123
```
As we can see, the return value is more complex: it consists of two nested lists:

 * A-T skew and then the skew for all positions (here 1 to 3).
 * C-G skew and then again the skew for all positions.
 
If we want to access the C-G skews for position 2 we simply can query:
```{r}
skews123$cg$`2`
```
...
Let us load a slightly larger DNA sequence in the fasta format that is stored in data/demo.fasta. The function `seq.fastaread()` reas a fasta file and returns the first entry.
```{r}
...
demoskews
```
There is a convenience command `nplet.skewplot` which plots the skew for different n-plet sizes.
```{r}
nplet.skewplot(demoskews,
               seqId = "demo sequence",
               yMax = .2)        # Range of y-axis
```
...
Calculate the sample-size normalized skews and plot them:
```{r}
#nsizes = c(5,10,15,20)
ndemoskews = nplet.sizenormskews(demoseq, nsizes)
nplet.normskewplot(ndemoskews, seqId = "demo sequence", yMax = 0.4)
```
...
```{r}
ndemoskews = nplet.normpartskews(demoseq, c(5,10,15,20))
nplet.normskewplot(ndemoskews, seqId = "demo sequence", yMax = 0.4)
```
 ...
 * The first entry encapsulated the A-T skews
  + then there is a list of partitions (here 1 to 4)
   - followed by the number of skews for each position in the n-plet (here 1,2,3)
...
nplet.partskewplot(demoseq, 
                   ...
                   yMax = .5)
```
Note that the x-axis shows now the partion (not the n-plet size).

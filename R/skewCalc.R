...
nplet.skewplot = function(nSkews, yMax = 1, seqId = "Unknown sequence") {
  ...
  boxplot(nSkews$at, main = seqId,
          ...
          xlab = "n-plet size", ylab = "A~T skew", xaxt="n")
  ...
  boxplot(nSkews$cg, main = seqId,
          ...
          xlab = "n-plet size", ylab = "C~G skew", xaxt="n")
...
#' n-plet sizes for a sequence stored in a fasta file.
#'
#' @param filename Path to fasta file.
#' @param nSizes Vector of n-plet sizes (e.g. 1,2,3)
#' @param yMax +/- max. y range.
#' @param skewF Function for the calcuation of the skew: either npletSkew
#' for the R version for npletSkewJ for the Java version. 
#' Default is npletSkewJ.
#'
#' @return
#' @export
#'
#' @examples
nplet.fileskewplot = function(filename,  nSizes = c(1,2,3), yMax = 1,
                              skewF = nplet.skewj) {
  seq = sequence.fastaread(filename)
  skews = nplet.skews(seq, nSizes, skewF)
  nplet.skewplot(seq, nSizes, yMax)
}
